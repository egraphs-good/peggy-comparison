...
/**
 * This class decodes the module types from the type table of an 
...
public class TypeTableDecoder {
	...
	private static void debug(String message) {
		if (DEBUG)
			System.err.println("TypeTableDecoder: " + message);
	}
	...
	private static Type parseType(HolderType[] typeTable, List<DataRecord> typeRecords, int index) {
		...
		case TypeBlock.TYPE_CODE_FUNCTION: {
		...
		case TypeBlock.TYPE_CODE_STRUCT: {
		...
		case TypeBlock.TYPE_CODE_ARRAY: {
		...
		case TypeBlock.TYPE_CODE_VECTOR: {
			long numelts = record.getOp(0).getNumericValue();
			Type elttype = typeTable[(int)record.getOp(1).getNumericValue()];
			if (elttype.isHolder()) {
	 ...
	 * encoded in the given TypeBlock. This method is smart enough to deal
	 * with forward references.
	 */
	public static Type[] decodeTypeTable(TypeBlock block) {
