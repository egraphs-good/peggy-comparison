...
/**
 * This class contains the blocks and records for a module block from an
...
public class ModuleBlock extends Block {
	...
	public ModuleBlock(EnterSubblock _enter) {
		super(_enter);
	}
	...
	protected void verifyContents(BlockContents bc) {
		if (bc.isBlock()) {
			...
			if (!(block.isParamAttr() ||
				  ...
				  block.isConstants() ||
				  block.isFunction())) {
				throw new IllegalArgumentException("Invalid subblock for MODULE_BLOCK: " + block.getBlockID());
			}
		} else {
	...
	public ModuleBlock getModuleSelf() {return this;}
}
