...
public class Nussinov {
    ...
    private static void initArray(int n, byte[] seq, int[][] table) {
        for (int i = 0; i < n; i++) {
            seq[i] = (byte) ((i + 1) % 4);
        }
    ...
    private static void printArray(int n, int[][] table) {
    ...
    private static int match(byte b1, byte b2) {
        return (b1 + b2) == 3 ? 1 : 0;
    }
    
    private static int maxScore(int s1, int s2) {
        return Math.max(s1, s2);
    }
    
    private static void kernelNussinov(int n, byte[] seq, int[][] table) {
        for (int i = n - 1; i >= 0; i--) {
            for (int j = i + 1; j < n; j++) {
                if (j - 1 >= 0) {
